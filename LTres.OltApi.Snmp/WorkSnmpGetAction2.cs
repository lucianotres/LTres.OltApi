    ...
    public async Task<WorkProbeResponse> Execute(WorkProbeInfo probeInfo, CancellationToken cancellationToken, WorkProbeResponse? initialResponse = null)
    {
        ...
        try
        {
            ...
            if (errorStatus == 0)
            {
                ...
                if (variable.Value is NoSuchInstance)
                    finalResponse.FailMessage = "NoSuchInstance";
                else if (variable.Value is NoSuchObject)
                ...
                else
                {
                    if (variable.Value is Integer32 integer)
                        ...
                        finalResponse.ValueStr = probeInfo.AsHex.GetValueOrDefault() ? str.ToHexString().Replace(" ", "") : str.ToString();
